...
File tracks everything needed to locate and identify a blob: `source`, `path`, `version`, `etag`, `size`, `is_latest`, and `last_modified`. This metadata is indexed by the Memory Engine and available for data-engine operations without touching the actual bytes.
