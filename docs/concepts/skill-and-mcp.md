...
| | Skill (OSS) | MCP (Studio / Memory Server) |
|---|---|---|
| Ships with | `pip install datachain` | Studio (hosted) or self-hosted Memory Server |
...
- **`dc-knowledge/`** — compiled markdown pages: schema, lineage, previews, links. The agent reads this first.
- **`.datachain/db`** — the SQLite dataset registry: every named, versioned dataset and its provenance. The agent queries this when the Knowledge Base summary is not enough.
...
## MCP (Studio / Memory Server)

When the team outgrows file sync, Data Memory moves into Studio (or a self-hosted Memory Server) and the agent reads through MCP — the open Model Context Protocol, with native client support in Claude Code, Cursor, and Codex. The Skill stays installed; it routes registry queries, schema introspection, and Data Memory reads through the MCP endpoint instead of the local filesystem. To set this up for your team, get in touch at [datachain.ai](https://datachain.ai).
