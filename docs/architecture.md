...
- **[Knowledge Base](concepts/knowledge-base.md)** compiles Data Memory into agent-readable pages: schema, lineage, previews, links. Reaches Claude Code, Cursor, and Codex through a Skill and an MCP server, architecturally separate from the Python library that holds the Compute Engine and Data Memory. Agents read here first; they query Data Memory when summaries are not enough; they trigger Compute Engine work only when nothing higher answers.
