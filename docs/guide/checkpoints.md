...
- **Class-based operation arguments:** Constructor arguments are part of a [class-based operation's](python-engine.md#class-based-lifecycle) checkpoint identity. For objects such as tokenizers or model clients, DataChain cannot know whether two instances behave identically, so it recomputes instead of risking an incorrect checkpoint result. If the object has a stable name or version, see [custom cache identity](../references/udf.md#cache-identity).
