...
| `dc.VideoFile` | `"video"` | -- | `.get_frames(step=N, video_stream_index=N)` → `VideoFrame[]`, `.get_fragments(duration)` → `VideoFragment[]`, `.get_info(video_stream_index=N)` → `dc.Video(fps,duration,codec,...)` |
...
Sub-file units:
- `VideoFrame` -- `.video_stream_index`, `.timestamp` seconds, `.get_np()` → ndarray, `.save(path)`
