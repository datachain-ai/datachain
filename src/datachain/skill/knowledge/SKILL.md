...
7. **NEVER bypass DataChain for results.** A task that runs ANY UDF (embeddings, LLM call, classification, file-content extraction) MUST land its output as a saved DataChain dataset. Writing results to a local `.json` / `.csv` / `.parquet` file via `open(...)`, `json.dump`, `pandas.to_csv`, or any Python-side file handle is a critical violation. The only legal way to materialise UDF output is `.save("name", attrs=[...], description=...)` on the chain.
8. **NEVER skip the C/A/S substrate when a UDF is involved.** Experiment is "persist by exception" — the **final Experiment-layer ranking / filter result** is what may be skipped, NOT the Sense / Asset layer underneath it. A similarity search needs a Sense layer of embeddings; an LLM-annotation task needs a Sense layer of annotations; a frame-extraction task needs an Asset layer of frames. These substrate layers are always built and saved (subject to the auto-build heuristic or the 3-option plan). Skipping them and computing everything in memory is the regression the methodology exists to prevent.
9. **Decompose multi-stage pipelines into one-script-per-stage.** When a CASE Decomposition produces 2+ named datasets (L1 + L2, or L2 + L3, or L1 + L2 + L3 + L4, …), the implementation MUST be one script per stage — NEVER a monolithic script with multiple `.save()` calls. Each script reads from the previous stage's saved dataset via `dc.read_dataset(...)` and writes its own with `.save("name", attrs=[...], description=...)`. Name each script after the dataset it produces (e.g., `build_l3_sense_dc_readme_clip_embeddings.py`, `find_dogs_similar_to_query.py`). Compressing stages into one script for speed violates the methodology: a failure in stage N forces every prior stage to re-run, and downstream reuse of any single stage requires excising code from the monolith. The core/SKILL.md "Code-level decomposition" section is the canonical reference; this rule promotes it to a Critical Rule because monolithic-pipeline-by-default is a recurring failure mode.
...
User questions almost always arrive Experiment-shaped on top of C/A/S substrate. The skill's job is to make the C/A/S substrate visible, reusable, and cheap. **"Persist by exception" applies only to the Experiment-layer final output (the ranking, the filter, the answer to one question). It does NOT extend backwards to the Sense / Asset / Container layers underneath — those are ALWAYS persisted as named DataChain datasets, never computed in memory and discarded.** A "one-off" similarity query is not an excuse to skip building the Sense layer; the embeddings are the load-bearing artifact of recall economics and exist precisely so the next session does not re-run them.
...
- **L4 (Experiment)** — persist by exception. Most ranking / similarity / filter outputs do not deserve a name. Save L4 only when the user explicitly asks, or when the result is a standing benchmark / training set that will be referenced again. **This exception applies ONLY to the final Experiment output, never to the C/A/S layers underneath.** If a similarity Experiment uses embeddings, the embeddings are a Sense layer and MUST be saved; only the ranking itself is optional.
...
5. **Foreground the whole-bucket option, and never silently narrow.** When proposing a layer build, list "build for the whole bucket" first. Even when the user's prompt names a subdirectory, the bucket root remains the default scope. Sample-only, skip-layer, and (only if the user pointed at a subdirectory) "this subdirectory only" come after the bucket-root option. NEVER silently auto-build a CAS layer at `scope:directory` without surfacing the choice — silent narrowing is the worst failure mode of the methodology because it produces a layer that looks reusable but isn't.
...
>    ```
>
>    Wait for the user's choice. The bucket-root option is mandatory. "This subdirectory only" exists ONLY as an explicit user opt-in.
>
>    **Common rationalizations that are NOT exceptions to the above.** If you find yourself thinking any of these, STOP and re-run 7a–7d from the top:
>    - *"The user's prompt is very task-specific, they probably want directory scope."* — Irrelevant. The bucket-root default applies regardless of how narrowly the prompt was phrased.
>    - *"The directory is small / cheap, so auto-build at directory scope is fine."* — No. Cost is measured at bucket-root scope (7b). Sub-second cost at directory scope is meaningless if you computed it at the wrong scope.
>    - *"The bucket is too big to auto-build, so I'll quietly do the directory."* — No. Surface the 3-option plan instead. Silent narrowing is the failure mode, not a workaround for a big bucket.
>    - *"It's a one-off question, no need to save the substrate."* — No. The C/A/S substrate is mandatory for any UDF-bearing task (Critical Rule 8). One-off applies only to the Experiment-layer ranking, never to the Sense / Asset / Container layers underneath.
>    - *"The dataset is tiny so I'll just compute in memory."* — No. See Critical Rule 7: UDF output lands as a saved DataChain dataset, period.
