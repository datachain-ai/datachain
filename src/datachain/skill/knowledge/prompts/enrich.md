...
- `attrs` (optional): a list of string tags from the DataChain record (set via `.save(attrs=[...])`). The skill encodes CAST methodology fields here: `cast:<layer>`, `scope:<bucket|directory|sample|onetime>`, `source:<slug>`, `parent:<dataset-name>` (the parent key may appear multiple times). Empty list `[]` is the default when no tags were set.
...
cast_layer: {container | asset | sense | task, or empty if not a CAST dataset}
cast_scope: {bucket | directory | sample | onetime, or empty}
cast_source: {bucket slug for L1-L3, task slug for L4, or empty}
cast_parents: [{comma-separated list of upstream CAST dataset names, or empty}]
---
...
- **CAST frontmatter fields are resolved in this order:**
  1. If the existing `.md` already has `cast_layer` / `cast_scope` / `cast_source` / `cast_parents` in its frontmatter, **preserve them verbatim** during re-enrichment (same rule as `## Session Context`).
  2. Otherwise, read the `attrs` list from the input JSON. If it contains a `cast:<layer>` tag, use that as `cast_layer`. Apply the same for `scope:<scope>` → `cast_scope`, `source:<slug>` → `cast_source`, and all `parent:<name>` entries → `cast_parents` (comma-separated list).
  3. If `attrs` carries no CAST tags, fall back to the dataset name. If the name matches `l1_container_…` / `l2_asset_…` / `l3_sense_…`, set `cast_layer` to container / asset / sense respectively. There is no name-prefix rule for Task (Task outputs use natural prefix-free names); they fall through to step 4. Leave `cast_scope`, `cast_source`, `cast_parents` empty.
  4. If neither `attrs` nor the name encodes a CAST layer, leave all four fields empty. The KB index will render the dataset under "Task Dataset" as the catch-all.
- **Conflict resolution.** If the name prefix and the `attrs` `cast:<layer>` tag disagree, prefer `attrs` silently — `attrs` is authoritative.
