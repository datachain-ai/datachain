...
- **CASE frontmatter fields are resolved in this order:**
  ...
  3. If `attrs` carries no CASE tags, fall back to the dataset name. If the name matches `l1_container_…` / `l2_asset_…` / `l3_sense_…` / `l4_experiment_…`, set `case_layer` accordingly. Leave `case_scope`, `case_source`, `case_parents` empty.
...
- **Conflict warning.** If the name prefix and the `attrs` `case:<layer>` tag disagree (e.g., name is `l3_sense_…` but `attrs` says `case:asset`), prefer `attrs` and add a single-line warning at the end of the `# {dataset_name}` description paragraph: `_Warning: name prefix and attrs disagree on CASE layer; frontmatter uses attrs._`
